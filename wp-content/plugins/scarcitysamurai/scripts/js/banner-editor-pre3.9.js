    ...
    banner.view.BannerElement = BannerElement = (function(_super) {
      ...
      BannerElement.prototype.initialize = function(options) {
        ...
        if (options != null ? options.editable : void 0) {
          ...
          this.elements.on('dragged', this._addEditor);
        }
      ...
      BannerElement.prototype._addEditor = function() {
        ...
        return tinyMCE.init({
          ...
          theme_advanced_buttons1: 'fontselect,forecolor,bold,italic,underline,strikethrough',
          theme_advanced_buttons2: '',
          theme_advanced_buttons3: '',
          theme_advanced_buttons4: '',
          theme_advanced_toolbar_location: 'external',
          theme_advanced_statusbar_location: 'none',
          theme_advacned_resizing: false,
          relative_urls: false,
          convert_urls: false,
          ...
          forced_root_block: '',
          setup: function(ed) {
            ed.onInit.add(_this._initEditor);
            ed.onEvent.add(_this._resizeEditor);
            ed.onExecCommand.add(_this._resizeEditor);
            ed.onKeyUp.add(_this._resizeEditor);
            return ed.onActivate.add(_this._resizeEditor);
          }
        });
      };
      ...
      BannerElement.prototype._initEditor = function(ed) {
        ...
        for (key in style) {
          ...
          if (key.match(/^color|font|line-height/)) {
            $(ed.contentDocument.body).css(key, value);
          }
        }
        ed.contentDocument.body.style.padding = 0;
        ed.contentDocument.body.style.margin = 0;
        ed.contentDocument.body.style.backgroundColor = 'transparent';
        ed.contentDocument.body.style.overflow = 'hidden';
        ed.contentDocument.body.style.whiteSpace = 'nowrap';
        this._resizeEditor();
        return $("#ss-banner-element-" + this.cid + "_external").appendTo($('.ss-banner-editor-layout-toolbar .ss-banner-editor-mce-toolbar'));
      };

      BannerElement.prototype._resizeEditor = function(ed) {
        var $block, h, w;
        ed = tinyMCE.getInstanceById("ss-banner-element-" + this.cid);
        this.model.set('data.text', ed.save());
        $block = $("#ss-banner-element-" + this.cid);
        $block.css('padding', '0 2px');
        w = Math.max(50, $block.width());
        if ($block.html() === '') {
          h = $block.text('DUMMY').height();
          $block.empty();
        } else {
          h = $block.height();
        }
        return $('iframe', ed.contentAreaContainer).css({
          height: "" + h + "px"
        }).attr('allowtransparency', 'true').closest('table').css({
          width: "" + (w + 4) + "px",
          height: "" + h + "px"
        });
      };
      ...
      BannerElement.prototype._removeEditor = function() {
        ...
        if (tinyMCE.getInstanceById(eid) != null) {
